...
export function AdminTelegramChat() {
  ...
  const entries = useMemo(() => {
    const all: ChatEntry[] = [
      { id: "ai", name: "AI помощник", sub: "Online · 24/7", ai: true },
      { id: "group", name: "Общий чат", sub: "31 участник · Admin + 30 компаний", group: true },
      ...companies.map((company) => ({ id: company.name, name: company.name, sub: "Личный чат" })),
    ];
    ...
    <div className={`panel tg ${showThread ? "show-thread" : ""}`}>
      <div className="tg-list">
        ...
        <div>
          {entries.map((item) => (
            <button
              key={item.id}
              className={`tg-item ${item.group || item.ai ? "is-group" : ""} ${item.id === current ? "active" : ""}`}
              type="button"
              onClick={() => {
                setCurrent(item.id);
                setShowThread(true);
      ...
      <div className="tg-main">
        <div className={`tg-head ${active?.group || active?.ai ? "is-group" : ""}`}>
          <button className="tg-back" type="button" onClick={() => setShowThread(false)} aria-label="Back">
            <ChevronLeft />
          </button>
        ...
        {current === "ai" ? (
          ...
          <>
            <ChatStream chat={current} isGroup={current === "group"} />
            <MessageComposer chat={current} onSent={() => qc.invalidateQueries({ queryKey: ["messages", current] })} />
          </>
        )}
      </div>
    </div>
